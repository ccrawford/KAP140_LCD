...
//Below for joystick inputs for auto-repeat pan/tilt
void RepeatButtonUpdate(const Button &btn) {
  if(btn.released()) {
    Serial.write(String(btn.get_name() + "=0\n").c_str());
  } else if(btn.held()) {
    delay(40); //joystick repeat click interval in mS, lower = faster panning
    Serial.write(String(btn.get_name() + "=1\n").c_str());
  }
}
...
Encoder ENC_COM_OUTER(31,30);
Encoder ENC_COM_INNER(27,29);
Encoder ENC_NAV_OUTER(57,55);
Encoder ENC_NAV_INNER(61,59);
...
Encoder ENC_FMS_INNER(4,2);
Encoder ENC_FMS_OUTER(43,45);
...
void setup() {
 ...
 // init non-mux buttons
  ...
  Buttons[i++].initialize("BTN_COM_VOL", 23, 10);  
